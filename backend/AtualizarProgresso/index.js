...
module.exports = async function (context, req) {
  ...
  try {
    ...
    const progressoExistente = await queryItems(
      "Progresso",
    ...
    if (progressoExistente.length > 0) {
      ...
      progressoDoc.exercicios.taxaAcerto =
        (progressoDoc.exercicios.corretos / progressoDoc.exercicios.tentados) *
        100

      progressoDoc.tempoEstudo.totalMinutos += Math.round(
        resultado.tempoSegundos / 60
      )
      progressoDoc.tempoEstudo.ultimaSessao = Math.round(
        resultado.tempoSegundos / 60
      )
