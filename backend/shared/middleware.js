...
const schemas = {
  cadastroResponsavel: Joi.object({
    responsavel: Joi.object({
      ...
      telefone: Joi.string().pattern(/^\+?55?\s?\(?[1-9]{2}\)?\s?9?\d{4}-?\d{4}$/).required(),
      parentesco: Joi.string().valid('Mãe', 'Pai', 'Avó', 'Avô', 'Tia', 'Tio', 'Outro').required()
    }).required(),
    aluno: Joi.object({
      nome: Joi.string().min(2).max(100).required(),
      dataNascimento: Joi.date().max('now').required(),
      serie: Joi.string().valid(
        '1º Ano', '2º Ano', '3º Ano', '4º Ano', '5º Ano',
        '6º Ano', '7º Ano', '8º Ano', '9º Ano'
      ).required(),
      escola: Joi.string().max(200).optional(),
      materias: Joi.array().items(Joi.string()).min(1).required(),
      livrosDidaticos: Joi.array().items(Joi.object({
