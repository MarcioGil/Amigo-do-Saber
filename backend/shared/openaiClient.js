...
function getOpenAIClient() {
  if (!openaiClient) {
    ...
    if (!apiKey) {
      throw new Error(
        "OPENAI_API_KEY ou AZURE_OPENAI_KEY deve estar configurado"
      )
    }
...
${
  contexto.dificuldades && contexto.dificuldades.length > 0
    ? `- Dificuldades conhecidas: ${contexto.dificuldades.join(", ")}`
    : ""
}`,
    })
  }
  ...
  try {
    ...
    return {
      resposta:
        "Oi querido! 🎀 Estou com um probleminha técnico agora, mas não se preocupe! Tente perguntar de novo em alguns segundos, tá bom? A Tia Dora está aqui pra te ajudar sempre! 💜",
      erro: true,
